...
class FeedFetcherTest: XCTestCase {
    ...
    func testThatItParse_9to5mac() {
        ...
        feed.feedURL = URL(string: "https://9to5mac.com/feed/")!
        ...
        XCTAssertEqual(firstArticle?.image.url?.absoluteString, "https://9to5mac.files.wordpress.com/2018/02/uber.jpg?quality=82&strip=all")
    ...
    func testThatItParse_iMore() {
        ...
        feed.feedURL = URL(string: "http://feeds.feedburner.com/TheIphoneBlog")!
        dataDownloader.dataContentsOfURLMock = rssData
        store.newArticlesReturned = []
        store.articleWithGUIDReturnMock = nil
        store.allFeedsReturnMock = [feed]
        ...
        XCTAssertEqual(firstArticle?.image.url?.absoluteString, nil)
